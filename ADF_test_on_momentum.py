...
# output paths
out_dir = "./sim_output_corrected_adf"
...
# ... (Plotting for log-returns is unchanged) ...
...
def run_adf_test(series, name):
    ...
    print(f"Testing spread on {name} data (points: {len(series)})...")
    ...
    if adf_result[1] < 0.05:
        print("  Result: p-value < 0.05. The spread is likely STATIONARY (cointegration is present).")
    else:
        print("  Result: p-value >= 0.05. The spread is likely NON-STATIONARY (no cointegration).")
    return adf_result[1]

# 1. Test on the "Training" data used to find n_hat
spread_train = spread_series.iloc[:n1]
p_train = run_adf_test(spread_train, "Training")

# 2. Test on the "Trading" (out-of-sample) data
spread_trade = spread_series.iloc[n1:]
p_trade = run_adf_test(spread_trade, "Trading (OOS)")
